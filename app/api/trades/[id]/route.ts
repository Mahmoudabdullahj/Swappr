...
export async function PATCH(
  ...
  if (status === 'completed') {
    ...
    const { error } = await db.from('trade_offers').update({ status: 'completed' }).eq('id', id);
  ...
  // accept / decline — verify current user is the recipient
  const { data: offer, error: fetchError } = await db
    .from('trade_offers')
    .select('*')
    .eq('id', id)
  ...
  const { error } = await db.from('trade_offers').update({ status }).eq('id', id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
